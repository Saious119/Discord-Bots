...
const playOnTheVoiceChannel = async (msg, voiceC, mp3File) => {
  ...
  try {
    if (randNum <= 20) {
      ...
      msg.channel.send(
        "OwO, You made a fuckey wuckey. Youwe gowing to tha Genewaw woice chat. UwU",
      );
    } else {
...
const caacCheck = async (msg) => {
  ...
  if (
    ...
    (msg.member.voice.channel === null ||
      msg.member.voice.channel?.name !== "caac")
  ) {
    await msg.channel.send({
      content:
        "You're not in the caac, only real gamers can use this when in the caac",
    });
    pass = false;
  }
...
client.on("messageCreate", async (msg) => {
  var NSFW_Channel = msg.guild.channels.cache.find(
    (NSFWch) => NSFWch.name === "nsfw",
  );
  var voiceC = msg.guild.channels.cache.find(
    (Voice) => Voice.name === "General",
  );
  ...
  if (msg.author.username == "UwUBot") {
    ...
    msg.content.includes("sugma") ||
    msg.content.includes("chokonma") ||
    msg.content.includes("boffa")
  ) {
    msg.channel.send("no UwU");
  } else if (
    msg.content.includes("UwUBot help") ||
    msg.content.includes("UwUBot what can you do")
  ) {
    msg.channel.send("I can do wots of thingy wingys, wike:");
    msg.channel.send("UwUify <statement> : make yoo woods kawaii UwU");
    msg.channel.send(
      "Woice chat : go to tha Genewaw woice chat and send me what you wanna hewe UwU",
    );
    ...
    msg.channel.send(
      "ask for an image or picture by saying image or picture OwO",
    );
    msg.channel.send("ok bye. UwU.");
  } else if (
    msg.content.includes("UwUsend") ||
    msg.content.includes("uwusend")
  ) {
    var msgSplit = msg.content.split(" ");
    if (msgSplit.length < 4) {
      msg.channel.send(
        "Pweese use da fowmat \'UwUsend <guildID> <channel ID> message\' k thx UwU",
      );
    }
  ...
  // AUDIO COMMANDS START
    ...
    msg.content.includes("patriotism") ||
    msg.content.includes("communism")
  ) {
    ...
    if (pass) {
      const wompWomps = msg.content
        .toLowerCase()
        .match(new RegExp("womp womp", "g")).length;
  ...
  // AUDIO COMMANDS END
    ...
    msg.channel.send(
      "ASMR, nuggets, UwU Bot sing me country music, UwU Bot Rap, UwU Bot are you drunk?",
    );
  ...
  // RANDOM RESPONSES START
    ...
    msg.channel.send(
      "*Pounces on you, notices your buldge* OwO, what's this? UwU",
    );
  ...
  // RANDOM RESPONSES END
    ...
    if (counter == 0) {
      counter = randint(10) + 5;
    }
  } else if (
    msg.content.includes("UwU Bot") ||
    msg.content.includes(" UwU Bot ")
  ) {
    msg.channel.send("*Is nervous* H-Hewwo UwU");
  } else if (
    msg.content.includes(" anime ") ||
    msg.content.includes(" anime's ")
  ) {
    ...
    msg.content.includes(" fuck ") ||
    msg.content.includes(" fucking ") ||
    msg.content.includes("Fucking") ||
    msg.content.includes("fuck")
  ) {
    msg.channel.send(
      "Oopsie woopsie, looks like we made a little fuckey wuckey, a little fucko boingo UwU",
    );
    if (counter == 0) {
      counter = randint(10) + 5;
    }
  } else if (
    msg.content.includes(" woops ") ||
    msg.content.includes("Woops") ||
    msg.content.includes("woops") ||
    msg.content.includes("whoops") ||
    msg.content.includes("Whoops")
  ) {
    msg.channel.send(
      "Oopsie woopsie UwU! It UwU looks like UwU I've dropped UwU some UwUs all over the UwU place UwU",
    );
...
async function getWompWomp() {
  ...
  try {
    ...
    while (
      !fileName.includes("womp") ||
      fileName.includes("cowboy") ||
      fileName.includes("long")
    ) {
      //must include womp but not cowboy womp or a long womp
